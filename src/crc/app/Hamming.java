    ...
    void fix() {
        ...
        if (errorWasDetected(mask)) {
            ...
            if (number < code.length) {
                if (type[number] == CORRECT)
                    type[number] = INCORRECT;              // korygujemy bit danych
                else if (type[number] == CORRECT_REDUNDANT)
                    type[number] = INCORRECT_REDUNDANT;    // korygujemy bit redundantny
